...
#include <stdlib.h>
#include <string.h>
char readByteR(char* pBuf,int* pos) {
	return pBuf[(*pos)--];
}
int readVIntR(char* pBuf,int* pos) {
...
int readVIntRP(char* pBuf, int* pos, const char* szName) {
	uint32_t i = readVIntR(pBuf, pos);
	printf("%s = %u\n", szName, i);
	return i;
}

char readByteRP(char* pBuf, int* pos, const char* szName) {
	char b = readByteR(pBuf, pos);
	printf("%s = %d\n", szName, b);
	return b;
}

void readBytes(unsigned char* dst, int offset, int len,char* src, int* pos) {
	for(int i=0;i<len;i++) {
...
int main() {
	...
	char *p = szFst;
	int numBytes = sizeof(szFst);
	int pos = numBytes - 1;
	readByteRP(szFst,&pos,"ARCS_AS_FIXED_ARRAY(0x20)");
	readVIntRP(szFst,&pos, "numArcs");
	readVIntRP(szFst,&pos, "bytesPerArc");
	printf("pos = %d\n",pos);

	return 0;

	//  BytesReader reader;
	int len = readVIntRP(szFst,&pos,"len");
	unsigned char* output = (unsigned char*)malloc(len);
	readBytes(output,0,len,szFst,&pos);
	printf("%s\n",output);
	{
